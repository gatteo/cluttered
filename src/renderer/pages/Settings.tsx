    ...
    <div className="h-screen flex flex-col">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-56 bg-surface-primary border-r border-white/5 p-4 flex flex-col">
          <button className="btn-subtle text-sm mb-4" onClick={goBack}>
            <ArrowLeft size={16} />
            Back
          </button>
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`w-full px-3 py-2 rounded-lg text-left flex items-center gap-3 transition-colors ${
                  activeTab === tab.id ? 'bg-accent-purple/20 text-white' : 'text-text-secondary hover:text-white hover:bg-white/5'
                }`}
                onClick={() => setActiveTab(tab.id)}
              >
                <span className="w-6 h-6 rounded-md bg-surface-elevated flex items-center justify-center">{tab.icon}</span>
                <span>{tab.label}</span>
              </button>
            ))}
          </nav>
