...
function ProtectedProjectModal({ project, onConfirm, onCancel }: { project: Project; onConfirm: () => void; onCancel: () => void }) {
  return (
    <motion.div
    ...
    >
      ...
      >
        ...
        <h3 className="text-lg font-semibold text-white mb-2">Select Protected Project?</h3>
        <p className="text-text-secondary text-sm mb-4">
          <span className="text-white font-medium">{project.name}</span> has uncommitted git changes. Cleaning this project will permanently delete
          your unsaved work.
        </p>
        <p className="text-text-muted text-xs mb-4">{project.path}</p>
        <div className="flex gap-3">
          <button onClick={onCancel} className="btn-ghost px-4 py-2 rounded-lg text-sm flex-1">
            Cancel
          </button>
          <button
            onClick={onConfirm}
...
export function EcosystemDetail({ ecosystem }: EcosystemDetailProps) {
  ...
  const allSelected = filteredProjects.length > 0 && filteredProjects.filter((p) => !p.isProtected).every((p) => selectedIds.has(p.id))
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-text-secondary">
                {filteredProjects.length} projects • <span className="text-white">{formatBytes(totalSize)}</span> total
                ...
                <Tooltip key={status} content={statusInfo[status].tooltip}>
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-full text-xs transition-all flex items-center gap-1.5 ${
                      isActive ? statusInfo[status].color : 'bg-white/5 text-text-muted'
                    }`}
                  >
          ...
          <div className="flex items-center gap-2">
            ...
            <Tooltip content="Show projects with uncommitted changes">
              <button
                ...
                className={`px-2.5 py-1 rounded-full text-xs transition-all flex items-center gap-1.5 ${
                  showProtected ? 'bg-amber-500/20 text-amber-400' : 'bg-white/5 text-text-muted'
                }`}
              >
            ...
            <Tooltip content="Show projects with no cleanable files">
              <button
                ...
                className={`px-2.5 py-1 rounded-full text-xs transition-all flex items-center gap-1.5 ${
                  showEmpty ? 'bg-white/20 text-white' : 'bg-white/5 text-text-muted'
                }`}
              >
            ...
            {filteredProjects.map((project, index) => (
              ...
              >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {project.isProtected && (
                      <Tooltip content={project.protectionReason || 'This project is protected from cleaning'}>
                        <span className="text-xs bg-amber-500/20 text-amber-400 px-2 py-0.5 rounded-full cursor-help">Protected</span>
                      </Tooltip>
                    )}
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-text-muted">{formatRelativeTime(project.lastModified)}</div>
                </div>
